...
%post
# set the layout because livecd-creator did not do that
# anaconda creates them after installation not before it
cat >/etc/sysconfig/keyboard << EOF
KEYBOARDTYPE="pc"
KEYTABLE="ar-qwerty"
LAYOUT="us,ara"
OPTIONS="grp_led:scroll,grp:alt_shift_toggle,altwin:super_win"
VARIANT=""
EOF
# setup monajat lang
cat >/etc/skel/.monajat-applet.rc <<EOF
lang=ar
EOF
...
# FIXME: why fedora did not stop it
...
snd_chs="PCM Surround Rear Front Center LFE Capture"
for card in /proc/asound/card[0-9]*; do
  c="\${card/\/proc\/asound\/card/}"
  for i in \$snd_chs; do amixer -c \$c sset \$i 100% 1>/dev/null; done
  amixer -c \$c | grep '^\S' | sed -re "s/^.*'([^']*)'.*$/\1/;" |
    while read i; do
       amixer -c \$c sset \$i 100% 1>/dev/null
    done
  amixer -c \$c sset Master 90% 1>/dev/null
  amixer -c \$c sset IEC958 on 1>/dev/null 2>/dev/null
done
...
for i in /etc/pki/rpm-gpg/RPM-GPG-KEY-*
do
